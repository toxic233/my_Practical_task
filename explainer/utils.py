    ...
    if args.dataset in ['Cora', 'CiteSeer', 'PubMed']:
        ...
        if args.dataset == 'DBLP':
            try:
                ...
                # 真实数据加载成功，直接使用adjM和features_list
                # adjM和features_list已经在load_DBLP_data中设置
            ...
            try:
                ...
                adjlists, edge_metapath_indices, features_list, adjM, type_mask, labels, train_val_test_idx = load_IMDB_data(prefix)
                
                print(f"成功加载真实IMDB数据集！")
                print(f"节点总数: {adjM.shape[0]}")
                print(f"边总数: {adjM.nnz // 2}")  # 无向图，所以除以2
                
                # 统计各类型节点数量
                unique_types, counts = np.unique(type_mask, return_counts=True)
                node_type_names = ['Movie', 'Director', 'Actor']
                for i, (node_type, count) in enumerate(zip(unique_types, counts)):
                    if i < len(node_type_names):
                        print(f"{node_type_names[i]}节点数量: {count}")
                
                # 将稀疏特征矩阵转换为密集数组
                features_list = [features.toarray() if hasattr(features, 'toarray') else features for features in features_list]
                
            except Exception as e:
                print(f"加载真实IMDB数据失败: {e}")
                print("使用模拟IMDB数据...")
        ...
        # 对于DBLP和IMDB真实数据，特征维度可能不同，需要统一处理
        if args.dataset in ['DBLP', 'IMDB'] and len(features_list) > 0:
            # 获取每种类型特征的维度
...
def load_model(args, data, device):
    ...
    if args.model == 'graphsage':
        ...
        if args.dataset == 'IMDB':
            ...
            print("加载真实IMDB数据集用于MAGNN模型...")
            from dataset.magnn_utils.data import load_IMDB_data
            
            # 加载真实的IMDB数据
            adjlists, edge_metapath_indices_list, features_list, adjM, type_mask, labels, train_val_test_idx = load_IMDB_data()
            
            # 获取节点数量
            num_movie = (type_mask == 0).sum()
            num_director = (type_mask == 1).sum()
            num_actor = (type_mask == 2).sum()
            total_nodes = len(type_mask)
            
            print(f"IMDB数据集统计: 电影={num_movie}, 导演={num_director}, 演员={num_actor}, 总节点数={total_nodes}")
            ...
            # 创建节点特征字典
            node_features_dict = {
                'movie': torch.FloatTensor(features_list[0]),
                'director': torch.FloatTensor(features_list[1]),
                'actor': torch.FloatTensor(features_list[2])
            }
            ...
            # adjlists包含[[G00, G01], [G10, G11], [G20, G21]]
            # edge_metapath_indices_list包含[[idx00, idx01], [idx10, idx11], [idx20, idx21]]
            ...
            # 生成movie-editor-movie元路径实例（模拟MEM元路径）
            print("生成movie-editor-movie元路径实例...")
            metapath_instances_mem = []
            
            # 由于没有真实的editor节点，我们通过演员或导演构建虚拟的MEM路径
            # 这里我们使用部分导演作为"编辑"角色，创建MEM元路径
            for movie_i in sampled_movies:
                if movie_i not in movie_to_directors:
                    continue
                ...
                # 选择部分导演作为"编辑"
                for director in directors[:1]:  # 只选择第一个导演作为编辑
                    if director not in director_to_movies:
                        continue
                    ...
                    # 添加MEM元路径实例 (movie-editor-movie)
                    for movie_j in movies:
                        if movie_i != movie_j:
                            # 使用type 1作为editor（与director相同，但概念上作为editor）
                            metapath_instances_mem.append([(0, movie_i), (1, director), (0, movie_j)])
                            
                            # 检查是否达到限制
                            if len(metapath_instances_mem) >= max_instances_per_type:
                                break
            ...
            print(f"生成的元路径实例数量: MDM={len(metapath_instances_mdm)}, MAM={len(metapath_instances_mam)}, MEM={len(metapath_instances_mem)}")
            
            # 合并元路径实例: MAM, MEM, MDM
            metapath_instances = [metapath_instances_mam, metapath_instances_mem, metapath_instances_mdm]
        ...
        if args.dataset == 'DBLP':
            ...
            # IMDB数据集特有的属性
            model.num_movie = num_movie
            model.num_director = num_director
            model.num_actor = num_actor
